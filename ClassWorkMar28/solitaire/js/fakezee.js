// constants
var TITLE_YPOS = 15;
var CARD_TRAY_POSITION = {x:78, y:250};
var CARD_DECK_POSITION = {x:78, y:100};
...
function preload() {
    ...
    queue.loadManifest([
		...
		{id:"cardsSpriteSheet", src:"img/cards.png"},
		{id:"cardsSheetData", src:"js/solitaire.json"}
...
function setupSpritesheet() {
	...
	cardsSpriteSheet = new createjs.SpriteSheet(queue.getResult('cardsSheetData'));
    initGame();
}
...
function buildTitle() {
	title = new createjs.Text("ANDRE GAME", "40px Arial", "#ffffff");
...
function buildCards(){
	...
	card.x = CARD_DECK_POSITION.x;
	card.y = CARD_DECK_POSITION.y;
	cardImg = new createjs.Sprite(cardsSpriteSheet, 'backCard');
	card.addChild(cardImg);
	stage.addChild(card);
	
	for(i=1; i<=4;i++){
		card = new createjs.Container();
		card.x = 250 + (CARD_DECK_POSITION.x + 20) * i;
		card.y = CARD_DECK_POSITION.y;
		cardImg = new createjs.Sprite(cardsSpriteSheet, 'emptyCard');
		card.addChild(cardImg);
		stage.addChild(card);
	}
	...
	for(i=1; i<=7;i++){
		card = new createjs.Container();
		card.x = (CARD_TRAY_POSITION.x + 13) * i;
		card.y = CARD_TRAY_POSITION.y;
		cardImg = new createjs.Sprite(cardsSpriteSheet, "one");
		card.addChild(cardImg);
		stage.addChild(card);
	}
